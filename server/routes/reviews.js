...
router.get('/written', authMiddleware, async (req, res) => {
  try {
    const db = await getDb();
    const rows = db.exec(`SELECT r.*, u.fullName as targetName, u.avatar as targetAvatar FROM reviews r JOIN users u ON r.toUserId = u.id WHERE r.fromUserId = '${req.user.id.replace(/'/g, "''")}' ORDER BY r.createdAt DESC`);
    if (!rows.length) return res.json({ reviews: [] });
    const cols = rows[0].columns;
    const reviews = rows[0].values.map(v => {
      const o = {};
      cols.forEach((c, i) => {
