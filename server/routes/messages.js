...
function pushToUser(userId, event, data) {
  ...
  if (!clients) return;
  // Send an unnamed `data:` event so the browser fires `onmessage`,
  // and embed the event type in the payload for the client to route on.
  const payload = `data: ${JSON.stringify(Object.assign({ type: event }, data))}\n\n`;
...
// EventSource cannot send Authorization headers, so the JWT is expected in the `token` query param.
