...
function _resolveApiBase() {
  // Check for explicit API URL meta tag first (used in production)
  var metaEl = document.querySelector('meta[name="api-base-url"]');
  if (metaEl && metaEl.getAttribute('content')) {
    return metaEl.getAttribute('content');
  }

  // Check for environment variable via a global config
  if (window.__TS_API_BASE) return window.__TS_API_BASE;

  var origin = window.location.origin;
  var protocol = window.location.protocol;

  // If served from a static host (GitHub Pages, file://, etc.), use production backend
  if (!origin || origin === 'null' || protocol === 'file:') {
    return 'http://localhost:5000/api';
  }

  // If same origin (served by Express backend), use relative URL
  return origin + '/api';
}
...
const api = {
  ...
  // Profile Photo
  uploadPhoto: function(imageBase64) {
    return new Promise(function(resolve, reject) {
      var token = api.token();
      fetch(API_BASE + '/users/profile/photo', {
        method: 'POST',
        headers: {
        ...
        if (err.name === 'TypeError' && /fetch|network/i.test(err.message)) {
          reject(new Error('Unable to connect to TeacherSwap.'));
        } else {
          reject(err);
        }
      });
  ...
  changePassword: function(currentPassword, newPassword) { return this.post('/users/change-password', { currentPassword, newPassword }); },

  // Settings
  getSettings: async function() { var d = await this.get('/users/settings'); return d.settings || {}; },
  updateSettings: function(settings) { return this.put('/users/settings', settings); },

  // Districts
  getDistrictsForRegion: async function(region) { var d = await this.get('/schools/districts?region=' + encodeURIComponent(region)); return d.districts || []; },
