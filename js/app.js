(function() {
  ...
  window.resolveAssetUrl = function(p) {
    if (!p) return '';
    if (/^https?:/i.test(p) || p.indexOf('data:') === 0) return p;
    var base = (typeof API_BASE === 'string' && API_BASE.indexOf('http') === 0)
      ? API_BASE.replace(/\/api$/, '')
      : '';
    return base + p;
  };

  window.avatarMarkup = function(user, cls) {
    var u = user || {};
    var src = window.resolveAssetUrl(u.avatar);
    var names = (u.fullName || u.username || '?').split(' ').filter(Boolean);
    var initials = (names[0] ? names[0].charAt(0) : '?') + (names[1] ? names[1].charAt(0) : '');
    var img = src
      ? '<img src="' + window.escapeHtml(src) + '" alt="' + window.escapeHtml(u.fullName || '') + '" onerror="this.parentNode.classList.add(\'ts-av-fallback-on\')">'
      : '';
    return '<span class="ts-av ' + (cls || '') + '">' + img + '<span class="ts-av-fallback">' + window.escapeHtml(initials) + '</span></span>';
  };
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // ============================
    // Confetti helper is exposed for inline submit handlers (register/dashboard).
    // Registration itself is handled by register.html's inline script, which
    // calls api.register() with the collected form data (single-step form).
    ...
    if (deleteAccountBtn) {
      deleteAccountBtn.addEventListener('click', async () => {
        if (typeof api === 'undefined' || !api.isLoggedIn()) {
          showToast(__('Please sign in first'), 'error');
          return;
        }
        if (!confirm(__('Are you sure you want to delete your account? This action cannot be undone.'))) return;
        deleteAccountBtn.disabled = true;
        try {
          showToast(__('Deleting account...'), 'info');
          ...
          showToast(err.message || __('Failed to delete account'), 'error');
        }
      });
    }
    ...
    document.querySelectorAll('.upload-preview input[type="file"]').forEach(input => {
      input.addEventListener('change', async function() {
        ...
        if (!/^image\/(png|jpe?g|gif|webp)$/.test(file.type)) {
          showToast(__('Please choose a PNG, JPG, GIF or WebP image.'), 'error');
          this.value = '';
          return;
        }
        if (file.size > 2 * 1024 * 1024) {
          showToast(__('Image too large (max 2MB).'), 'error');
          this.value = '';
          return;
        }
        ...
        reader.onload = async (e) => {
          ...
          const host = this.closest('.upload-preview');
          try {
            if (typeof api === 'undefined' || !api.isLoggedIn()) {
              showToast(__('Sign in to save your profile photo.'), 'info');
              return;
            }
            ...
            api.saveUser(user);
            if (host) {
              const oldAvatarImg = host.querySelector('.avatar-img');
              if (oldAvatarImg) oldAvatarImg.remove();
              host.classList.add('has-avatar');
              const imgEl = document.createElement('img');
              imgEl.className = 'avatar-img';
              imgEl.alt = user.fullName || '';
              imgEl.src = window.resolveAssetUrl(res.avatar);
              host.appendChild(imgEl);
            }
            showToast(__('Profile photo updated.'), 'success');
            if (window.onAvatarUploaded) window.onAvatarUploaded(res.avatar);
          } catch (err) {
            if (host) { this.value = ''; }
            showToast(err.message || __('Failed to upload photo'), 'error');
          }
        };
    ...
    if (particlesContainer) {
      ...
      for (var j = 0; j < 3; j++) {
        ...
        var cSize = Math.random() * 100 + 80;
        var cX = Math.random() * 100;
        var cY = Math.random() * 100;
        var cDelay = Math.random() * 8;
        var cDuration = Math.random() * 6 + 8;
        circle.style.cssText = 'width: ' + cSize + 'px; height: ' + cSize + 'px; left: ' + cX + '%; top: ' + cY + '%; border-color: var(--primary); animation-delay: ' + cDelay + 's; animation-duration: ' + cDuration + 's;';
